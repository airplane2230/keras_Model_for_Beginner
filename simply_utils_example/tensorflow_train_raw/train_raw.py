...
for epoch in range(EPOCHS):
    ...
    LR_for_print = LRSchedule(init_lr,warmup_epoch=warmup_epoch,decay_fn=lr_scheduler,init_epoch=init_epoch)
    
    print('current learning_rate: ', LR_for_print(epoch + 1).numpy())
    ...
    for (batch, (tensor, target)) in tqdm_dataset:
        ...
        tqdm_dataset.set_postfix({
            ...
            'Accuracy': '{:04f}'.format(train-accuracy.result().numpy())
        })
        
    loss_plot.append(total_loss_format)
    
    if np.min(loss_plot) == loss_plot[-1]:
        print(f'{epoch + 1} - min Loss!: {loss_plot[-1]}, and Accuracy: {train_accuracy.result().numpy()}')
